...
public interface SantePourTousDao {
    ...
    @Query("SELECT * FROM santepourtous WHERE type = 1 ORDER BY titre ASC")
    LiveData<List<SantePourTous>> getAllMaladieItems();

    @Query("SELECT * FROM santepourtous WHERE type = 2 ORDER BY titre ASC")
    LiveData<List<SantePourTous>> getAllBienEtreItems();

    @Query("SELECT * FROM santepourtous WHERE  firebase_id LIKE :firebasekey")
    LiveData<SantePourTous> getSantePourTousByFirebaseId(String firebasekey);
